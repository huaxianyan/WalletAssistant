...
- Gmail Token、原始邮件、候选邮件、导入截图、OCR 文字和解析结果只在设备端处理与保存，不发送到自建服务端
- Google Wallet 卡片只展示乘车所需的基本信息，不处理、复制或模拟 12306 动态检票二维码
- Android 应用是行程管理和提醒的主要入口；应用通知默认关闭，并支持新行程默认值与单个行程设置
- 中国铁路通知的状态栏小图标固定使用专用的单色火车图形；通知卡片不添加大图标，应用身份图标由 Android 系统展示；未来其他行程来源使用各自的通知小图标
- 通知中的「已上车」会结束提醒并归档，「取消提醒」只结束提醒，不得取消独立的自动归档任务
